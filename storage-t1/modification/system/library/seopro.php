...
class SeoPro {
    ...
    public function prepareRoute($parts) {
		...
		$a_parts = $parts; // !A because it will be deleted before my code in SeoPro
        ...
        if (!empty($parts) && is_array($parts)) {
            ...
            if(!$query) {
				...
				// Check up all $parts, not only $last_part
				$in = '';
				...
				if ($res) {
					...
					// So I don't want support it
					// if ('blog_category_id' == $res[0]) {
					// 	$new_url = $this->url->link('blog/category', 'blog_category_id=' . $res[1]);
					// }

					// if ('article_id' == $res[0]) {
					// 	$new_url = $this->url->link('blog/article', 'article_id=' . $res[1]);
					// }
					// ocStore only . end
