	...
	public function index() {
		...
		if ($product_info) {
			...
			if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
				...
				$data['price'] = number_format((float)$data['price'], 0, '.', ' ');
			...
			if (!is_null($product_info['special']) && (float)$product_info['special'] >= 0) {
				...
				$data['special'] = number_format((float)$data['special'], 0, '.', ' ');
			...
			if ($this->config->get('config_tax')) {
				...
				$data['tax'] = number_format((float)$tax_price, 0, '.', ' ');
				$data['tax'] = $data['tax'] . " ₽";
			} else {
				$data['tax'] = false;
			}
			...
			$data['rating_text'] = $this->numWords($data['rating'], array($rating_text_1, $rating_text_2, $rating_text_3));
			...
			$filter_data['filter_name'] = $product_info['name'];
			$results = $this->model_catalog_product->getProducts($filter_data, $product_info['product_id']);
			...
			$seller = $this->model_catalog_product->getSeller($this->request->get['product_id']);
			if ($seller) {
				$data['seller_name'] = $seller['name'];
				$data['seller_address'] = $seller['address'];
			} else {
				$data['seller_name'] = $product_info['manufacturer'];
				$data['seller_address'] = $product_info['location'];
			}

			$data['compare'] = $this->model_catalog_product->getProductCompare($result['product_id']);
			...
			$this->model_catalog_product->updateViewed($this->request->get['product_id']);
                    ...
                    $this->document->addStyle('catalog/view/theme/default/stylesheet/kbmp_marketplace/product-page.css');
                    ...
                    $data['text_more_seller_products'] = $this->language->get('text_more_seller_products');
                    ...
                    if (isset($settings['kbmp_marketplace_setting']['kbmp_module_enable']) && $settings['kbmp_marketplace_setting']['kbmp_module_enable']) {
                        if (isset($settings['kbmp_marketplace_setting']['kbmp_enable_seller_details']) && $settings['kbmp_marketplace_setting']['kbmp_enable_seller_details']) {
                            $seller_details = $this->model_kbmp_marketplace_kbmp_marketplace->getProductSeller($this->request->get['product_id']);
                            if (isset($seller_details) && !empty($seller_details)) {
                                $data['seller_products_link'] = $this->url->link('kbmp_marketplace/sellers/view', '&seller_id=' . $seller_details['seller_id']);

                                $data['seller_details'] = $seller_details;
                                //Get Seller Reviews details
                                $seller_review_filter = array(
                                    'seller_id' => $data['seller_details']['seller_id'],
                                    'filter_status' => 1
                                );
                                $data['seller_total_reviews'] = sprintf($data['text_total_seller_reviews'], $this->model_kbmp_marketplace_kbmp_marketplace->getTotalSellerReviews($seller_review_filter));
                                $data['seller_rating'] = $this->model_kbmp_marketplace_kbmp_marketplace->getSellerRating($seller_review_filter) * 20;
                                $data['seller_reviews_link'] = $this->url->link('kbmp_marketplace/sellers/reviews', '&seller_id=' . $data['seller_details']['seller_id']);
                            }
                        }
                    }
      ...
      // OCFilter End
			...
			if ($cant_add)
				$data['button_cart'] = $existLang;
			else
				$data['button_cart'] = $notExistLang;
			
			if ($data['quantity'] == 0)
				$data['button_cart'] = $existLangZero;
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!isset($json['error'])) {
				...
				$this->model_catalog_review->addReview($this->request->get['product_id'], $this->request->post);
                    ...
                    $seller_details = $this->model_kbmp_marketplace_kbmp_marketplace->getProductSeller($this->request->get['product_id']);
                    if (isset($seller_details) && !empty($seller_details)) {
                        $this->model_kbmp_marketplace_kbmp_marketplace->addSellerProductReview($this->request->get['product_id'], $seller_details['seller_id']);
                    }
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            ...
            if ($product_info) {
			    ...
			    if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
					...
					$json['price'] = number_format((float)$json['price'], 0, '.', ' ');
			    ...
			    if ((float)$product_info['special']) {
				    $json['special'] = $this->currency->format($this->tax->calculate($product_info['special'] + $options_price, $product_info['tax_class_id'], $this->config->get('config_tax')) * $quantity, $this->session->data['currency']);
					$json['special'] = number_format((float)$json['special'], 0, '.', ' ');
			    ...
			    if ($this->config->get('config_tax')) {
				    ...
				    $json['tax'] = $this->language->get('text_tax') . ' ' . $this->currency->format($tax, $this->session->data['currency']);
					$json['tax'] = number_format((float)$tax, 0, '.', ' ');
					$json['tax'] = $this->language->get('text_tax') . " " . $json['tax'] . " ₽";
			    } else {
				    $json['tax'] = false;
			    }
			    ...
			    foreach ($this->model_catalog_product->getProductDiscounts($product_id) as $discount) {
				    ...
				    $json['discounts'][] = $discount['quantity'] . $this->language->get('text_discount') . $discount_price;
			    }
            }
        }
