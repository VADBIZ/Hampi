...
class ModelCheckoutOrder extends Model {
		...
		if (isset($data['totals'])) {
			foreach ($data['totals'] as $total) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_id = '" . (int)$order_id . "', code = '" . $this->db->escape($total['code']) . "', title = '" . $this->db->escape($total['title']) . "', `value` = '" . (float)$total['value'] . "', sort_order = '" . (int)$total['sort_order'] . "'");
                    ...
                    //Update Commisson/Earning.
                    $this->load->model('kbmp_marketplace/kbmp_marketplace');
                    $this->model_kbmp_marketplace_kbmp_marketplace->setSellersEarning($order_id);
                      //Update Shipping value of Marketplace Orders
                      //Start by dharmanshu 25-02-2021 for the seller shipping issue
                    $shipping_geo_zone_id = '';
                    if (isset($data['shipping_code'])) {
                        if (strpos($data['shipping_code'], 'marketplace_custom_shipping.marketplace_custom_shipping_') !== false) {
                            $shipping_geo_zone_id = str_replace("marketplace_custom_shipping.marketplace_custom_shipping_", "", $data['shipping_code']);

                            $this->model_kbmp_marketplace_kbmp_marketplace->updateOrderShipping($order_id, $shipping_geo_zone_id);
                        }
                    }
                    //End by dharmanshu 25-02-2021 for the seller shipping issue
	...
	public function deleteOrder($order_id) {
		...
		$this->db->query("DELETE `or`, ort FROM `" . DB_PREFIX . "order_recurring` `or`, `" . DB_PREFIX . "order_recurring_transaction` `ort` WHERE order_id = '" . (int)$order_id . "' AND ort.order_recurring_id = `or`.order_recurring_id");
                    ...
                    //Delete Commisson/Earning.
                    $this->load->model('kbmp_marketplace/kbmp_marketplace');
                    $this->model_kbmp_marketplace_kbmp_marketplace->deleteSellersEarning($order_id);
