...
class ControllerCatalogProduct extends Controller {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_catalog_product->addProduct($this->request->post);
                    ...
                    if($seller_id) {
                        ...
                        $product_id = $this->model_kbmp_marketplace_kbmp_marketplace->getProductId();
                        $store_id = (int) $this->config->get('config_store_id');
                        $settings = $this->model_setting_setting->getSetting('kbmp_marketplace', $store_id);
                        if (!empty($product_id)) {
                            $this->model_kbmp_marketplace_kbmp_marketplace->addSellerProduct($seller_id, $product_id, $settings['kbmp_marketplace_setting']['kbmp_new_product_approval_required']);
                            $this->model_kbmp_marketplace_kbmp_marketplace->approveProduct($product_id);
                        }
                    }
			...
			if (isset($this->request->get['page'])) {
				$url .= '&page=' . $this->request->get['page'];
                        ...
                        if (isset($this->request->get['redirect']) && $this->request->get['redirect'] == 'products_list') {
                            $this->response->redirect($this->url->link('kbmp_marketplace/products_list', 'user_token=' . $this->session->data['user_token'], true));
                        } else if (isset($this->request->get['redirect']) && $this->request->get['redirect'] == 'products_approval_list') {
                            $this->response->redirect($this->url->link('kbmp_marketplace/products_approval_list', 'user_token=' . $this->session->data['user_token'], true));
                        }
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
                        ...
                        //Check if product is seller product and approved then allow to edit
                        $sellerProduct = $this->model_kbmp_marketplace_kbmp_marketplace->isSellerProduct($this->request->get['product_id']);
                        if (isset($sellerProduct['approved']) && $sellerProduct['approved'] != '1') {
                            $this->session->data['error'] = $this->language->get('text_unapproved_product_edit_error');

                            if (isset($this->request->get['redirect']) && !empty($this->request->get['redirect'])) {
                                $this->response->redirect($this->url->link('kbmp_marketplace/'.$this->request->get['redirect'], 'user_token=' . $this->session->data['user_token'], true));
                            }
                        } else if (isset($sellerProduct['seller_id'])) {
                            //Check if seller approved or not
                            $sellerDetails = $this->model_kbmp_marketplace_kbmp_marketplace->getSellerAccountDetails($sellerProduct['seller_id']);
                            if (isset($sellerDetails['approved']) && $sellerDetails['approved'] != '1') {
                                $this->session->data['error'] = $this->language->get('text_unapproved_product_edit_error');
			...
			$this->model_catalog_product->editProduct($this->request->get['product_id'], $this->request->post);
                    ...
                    if($this->request->post['disabled_status'] == 0 && !empty($this->request->post['seller'])){
                        $seller_id = $this->request->post['seller'];
                        $this->load->model('kbmp_marketplace/kbmp_marketplace');
                        $this->load->model('setting/setting');
	...
	protected function getList() {
		...
		*/
		...
		$data['order'] = $order;
                    ...
                    $this->load->language('kbmp_marketplace/common');
                    $this->load->model('kbmp_marketplace/kbmp_marketplace');
                    $data['seller_select_label'] = $this->language->get('seller_select_label');
                    $data['seller'] = $this->language->get('seller');
                    if (isset($this->request->post['seller'])) {
                        $data['seller_value'] = $this->request->post['seller'];
                    } else {
                        $data['seller_value'] = '';
                    }
                    $seller_product_list = $this->model_kbmp_marketplace_kbmp_marketplace->getSellersProducts();
                    if(isset($this->request->get['product_id']) && !empty($seller_product_list)) {
                        foreach($seller_product_list as $seller_product) {
                            if($seller_product['product_id'] == $this->request->get['product_id']) {
                                $data['disabled_status'] = 1;
                                $data['seller_value'] = $seller_product['seller_id'];
                            }
                        }
                    }
                    $results = $this->model_kbmp_marketplace_kbmp_marketplace->getSellers([]);
                    $data['sellers'] = [];
                    foreach ($results as $result) {
                        $data['sellers'][] = array(
    ...
    // SEO URL Generator FREE . begin
        ...
        if (!empty($product_info)) {
			$data['indexnow'] = $product_info['indexnow'];
		} else {
			$data['indexnow'] = '';
		}
		...
		if (isset($this->request->get['product_id'])) {
			if (isset($this->request->post['seo_url_generator_redirects'])) {
				...
				$data['redirects'] = $this->model_extension_module_seo_url_generator->getRedirects('product_id', $this->request->get['product_id']);
			}
		} else {
