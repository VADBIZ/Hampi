...
class ControllerCustomerCustomer extends Controller {
			...
			if (isset($this->request->get['page'])) {
				$url .= '&page=' . $this->request->get['page'];
                    ...
                    if (isset($this->request->get['redirect']) && $this->request->get['redirect'] == 'sellers_list') {
                        $this->response->redirect($this->url->link('kbmp_marketplace/sellers_list', 'user_token=' . $this->session->data['user_token'], true));
                    } else if (isset($this->request->get['redirect']) && $this->request->get['redirect'] == 'sellers_account_approval_list') {
                        $this->response->redirect($this->url->link('kbmp_marketplace/sellers_account_approval_list', 'user_token=' . $this->session->data['user_token'], true));
                    }
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_customer_customer->editCustomer($this->request->get['customer_id'], $this->request->post);
                    ...
                    $this->load->model('kbmp_marketplace/kbmp_marketplace');
                    $is_seller = $this->model_kbmp_marketplace_kbmp_marketplace->is_seller($this->request->get['customer_id']);
                    if (isset($is_seller) && $is_seller) {
                        //Get Seller ID by Customer ID
                        $seller = $this->model_kbmp_marketplace_kbmp_marketplace->getSellerByCustomerId($this->request->get['customer_id']);
                        $this->model_kbmp_marketplace_kbmp_marketplace->updateSellerConfig($this->request->post, $seller['seller_id'], $seller['store_id']);
                    }
                        ...
                        if(isset($this->request->get['customer_id'])){
                            ...
                            $seller_check = $this->model_kbmp_marketplace_kbmp_marketplace->check_seller($customer_id);
                            if($seller_check){
                                $this->model_kbmp_marketplace_kbmp_marketplace->approveseller_change($customer_id,$seller);
                            }
                            else{
                                $store_id = (int) $this->config->get('config_store_id');
                                $settings = $this->model_setting_setting->getSetting('kbmp_marketplace', $store_id);
    ...
    public function disapprove() {
        ...
        if (isset($this->request->post['selected']) && $this->validateDisapprove()) {
			foreach ($this->request->post['selected'] as $customer_id) {
				$this->model_customer_customer->disapproveCustomer($customer_id);
			}
	...
	protected function getList() {
        ...
        $data['approve'] = $this->url->link('customer/customer/approve', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['disapprove'] = $this->url->link('customer/customer/disapprove', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$data['order'] = $order;
                    ...
                    if(isset($this->request->post['seller'])){
                        $data['approved'] = $this->request->post['seller'];
                    } else if(isset($this->request->get['customer_id'])) {
                        $customer_id = $this->request->get['customer_id'];
                        $this->load->model('kbmp_marketplace/kbmp_marketplace');
                        $seller_data = $this->model_kbmp_marketplace_kbmp_marketplace->get_sellerData($customer_id);
                        if(!empty($seller_data)){
                            $data['approved'] = $seller_data['approved'];
                        }
                    }
	...
	protected function getForm() {
                    ...
                    /* Start Dharmasnhu for the seller register */
                    $this->load->model('kbmp_marketplace/kbmp_marketplace');
                    $data['seller'] = $this->language->get('seller');
                    $data['register'] = $this->language->get('register');
                    $data['yes_status'] = $this->language->get('yes_status');
                    $data['no_status'] = $this->language->get('no_status');
                    /* End Dharmasnhu for the seller register */
                    ...
                    if (isset($is_seller) && $is_seller) {                           
                        ...
                        $data['label_seller_review_approval_required'] = $this->language->get('label_seller_review_approval_required');
                        $data['label_send_email_order_place'] = $this->language->get('label_send_email_order_place');
                        ...
                        $data['text_use_global'] = $this->language->get('text_use_global');

                        //Get Seller ID by Customer ID
                        $seller = $this->model_kbmp_marketplace_kbmp_marketplace->getSellerByCustomerId($this->request->get['customer_id']);
                        $seller_config = $this->model_kbmp_marketplace_kbmp_marketplace->getSellerConfig($seller['seller_id']);

                        if (isset($seller_config) && !empty($seller_config)) {
                            foreach ($seller_config as $sellerconfig) {
                                $data['seller_config'][$sellerconfig['key']] = $sellerconfig['value'];
                                $data['seller_config'][$sellerconfig['key'].'_global'] = $sellerconfig['use_global'];
                            }
                        }

                        if (isset($this->request->post['kbmp_default_commission'])) {
                            $data['kbmp_default_commission'] = $this->request->post['kbmp_default_commission'];
                        } elseif (isset($data['seller_config']['kbmp_default_commission']) && !empty($data['seller_config']['kbmp_default_commission'])) {
                            $data['kbmp_default_commission'] = $data['seller_config']['kbmp_default_commission'];
                        } else {
                            $data['kbmp_default_commission'] = '';
                        }

                        if (isset($this->request->post['kbmp_default_commission_global'])) {
                            $data['kbmp_default_commission_global'] = $this->request->post['kbmp_default_commission_global'];
                        } elseif (isset($data['seller_config']['kbmp_default_commission_global']) && !empty($data['seller_config']['kbmp_default_commission_global'])) {
                            $data['kbmp_default_commission_global'] = $data['seller_config']['kbmp_default_commission_global'];
                        } else {
                            $data['kbmp_default_commission_global'] = '';
                        }

                        if (isset($this->request->post['kbmp_new_product_approval_required'])) {
                            $data['kbmp_new_product_approval_required'] = $this->request->post['kbmp_new_product_approval_required'];
                        } elseif (!empty($data['seller_config']['kbmp_new_product_approval_required'])) {
                            $data['kbmp_new_product_approval_required'] = $data['seller_config']['kbmp_new_product_approval_required'];
                        } else {
                            $data['kbmp_new_product_approval_required'] = '0';
                        }

                        if (isset($this->request->post['kbmp_new_product_approval_required_global'])) {
                            $data['kbmp_new_product_approval_required_global'] = $this->request->post['kbmp_new_product_approval_required_global'];
                        } elseif (!empty($data['seller_config']['kbmp_new_product_approval_required_global'])) {
                            $data['kbmp_new_product_approval_required_global'] = $data['seller_config']['kbmp_new_product_approval_required_global'];
                        } else {
                            $data['kbmp_new_product_approval_required_global'] = '';
                        }

                        if (isset($this->request->post['kbmp_enable_seller_review'])) {
                            $data['kbmp_enable_seller_review'] = $this->request->post['kbmp_enable_seller_review'];
                        } elseif (!empty($data['seller_config']['kbmp_enable_seller_review'])) {
                            $data['kbmp_enable_seller_review'] = $data['seller_config']['kbmp_enable_seller_review'];
                        } else {
                            $data['kbmp_enable_seller_review'] = '0';
                        }

                        if (isset($this->request->post['kbmp_enable_seller_review_global'])) {
                            $data['kbmp_enable_seller_review_global'] = $this->request->post['kbmp_enable_seller_review_global'];
                        } elseif (!empty($data['seller_config']['kbmp_enable_seller_review_global'])) {
                            $data['kbmp_enable_seller_review_global'] = $data['seller_config']['kbmp_enable_seller_review_global'];
                        } else {
                            $data['kbmp_enable_seller_review_global'] = '0';
                        }

                        if (isset($this->request->post['kbmp_seller_review_approval_required'])) {
                            $data['kbmp_seller_review_approval_required'] = $this->request->post['kbmp_seller_review_approval_required'];
                        } elseif (!empty($data['seller_config']['kbmp_seller_review_approval_required'])) {
                            $data['kbmp_seller_review_approval_required'] = $data['seller_config']['kbmp_seller_review_approval_required'];
                        } else {
                            $data['kbmp_seller_review_approval_required'] = '0';
                        }

                        if (isset($this->request->post['kbmp_seller_review_approval_required_global'])) {
                            $data['kbmp_seller_review_approval_required_global'] = $this->request->post['kbmp_seller_review_approval_required_global'];
                        } elseif (!empty($data['seller_config']['kbmp_seller_review_approval_required_global'])) {
                            $data['kbmp_seller_review_approval_required_global'] = $data['seller_config']['kbmp_seller_review_approval_required_global'];
                        } else {
                            $data['kbmp_seller_review_approval_required_global'] = '0';
                        }

                        if (isset($this->request->post['kbmp_email_on_new_order'])) {
                            $data['kbmp_email_on_new_order'] = $this->request->post['kbmp_email_on_new_order'];
                        } elseif (!empty($data['seller_config']['kbmp_email_on_new_order'])) {
                            $data['kbmp_email_on_new_order'] = $data['seller_config']['kbmp_email_on_new_order'];
                        } else {
                            $data['kbmp_email_on_new_order'] = '0';
                        }

                        if (isset($this->request->post['kbmp_email_on_new_order_global'])) {
                            $data['kbmp_email_on_new_order_global'] = $this->request->post['kbmp_email_on_new_order_global'];
                        } elseif (!empty($data['seller_config']['kbmp_email_on_new_order_global'])) {
                            $data['kbmp_email_on_new_order_global'] = $data['seller_config']['kbmp_email_on_new_order_global'];
                        } else {
                            $data['kbmp_email_on_new_order_global'] = '0';
                        }
                        ...
                        if (isset($this->request->post['product_category'])) {
                        ...
                        } elseif (!empty($data['seller_config']['product_category'])) {
                            $categories = explode(", ", $data['seller_config']['product_category']);
                            foreach ($categories as $category_id) {
                                $category_info = $this->model_catalog_category->getCategory($category_id);
                            ...
                            $seller_categories_list = array();
                            $seller_categories = $this->model_kbmp_marketplace_kbmp_marketplace->getAssignedCategories($seller['seller_id']);
                            if (isset($seller_categories) && !empty($seller_categories)) {
                                foreach ($seller_categories as $seller_category) {
                                    $seller_categories_list[] = $seller_category['category_id'];
                                }
                            }

                            if (isset($seller_categories_list) && !empty($seller_categories_list)) {
                                $categories = $seller_categories_list;
                                foreach ($categories as $category_id) {
                                    $category_info = $this->model_catalog_category->getCategory($category_id);
		...
		if (isset($this->error['custom_field'])) {
			...
			$data['error_custom_field'] = array();
                    ...
                    if (isset($this->error['kbmp_default_commission'])) {
                        $data['error_kbmp_default_commission'] = $this->error['kbmp_default_commission'];
                    } else {
                        $data['error_kbmp_default_commission'] = array();
                    }
		...
		if (!isset($this->request->get['customer_id'])) {
                    ...
                    if (isset($this->request->get['redirect']) && !empty($this->request->get['redirect'])) {
                        $url .= '&redirect=' . $this->request->get['redirect'];
                    }
		...
		$data['footer'] = $this->load->controller('common/footer');
                    ...
                    $data['marketplace_enable'] =$this->model_setting_kbmp_marketplace->checkInstalled('kbmp_marketplace');
                    if($data['marketplace_enable'] == 1){
                        $this->load->model('setting/setting');
                        $kbmp_marketplace = $this->model_setting_setting->getSetting('kbmp_marketplace');
                        if(isset($kbmp_marketplace['kbmp_marketplace_setting']['kbmp_module_enable'])){
                            $data['marketplace_enable'] = $kbmp_marketplace['kbmp_marketplace_setting']['kbmp_module_enable'];
                        }
                    }
		...
		if (isset($this->request->post['seller_field'])) {
			...
			if (count($error_seller_field) > 0)
				$this->error['error_seller_field'] = $error_seller_field;
                    ...
                    $this->load->language('kbmp_marketplace/common'); 
                    if (isset($this->request->post['kbmp_default_commission']) && (utf8_strlen($this->request->post['kbmp_default_commission']) < 1 || $this->request->post['kbmp_default_commission'] < 0 || $this->request->post['kbmp_default_commission'] > 100)) {
                        $this->error['kbmp_default_commission'] = $this->language->get('error_kbmp_default_commission');
                    }
                    $this->load->language('customer/customer');
