		...
		if ($order_info) {
			...
			foreach ($products as $product) {
				...
				foreach ($options as $option) {
                    ...
                    //Code added by gopi to resolve the issue of trackin no. 27 feb--start
                    ...
                    if (isset($settings['kbmp_marketplace_setting']['kbmp_module_enable']) && $settings['kbmp_marketplace_setting']['kbmp_module_enable']) {
                        $data['text_tracking_column'] = $this->language->get('text_tracking_column');
                        $data['tracking_column'] = 1;
                        $tracking_number = $this->model_kbmp_marketplace_kbmp_marketplace->getProductTracking($order_id,$product['product_id']);
                        //inserting data in default product array
                        $data['products'][] = array(
                                ...
                                'href'     		   => $this->url->link('catalog/product/edit', 'token=' . $this->session->data['user_token'] . '&product_id=' . $product['product_id'], true)
                            );
                            continue;       
                    } else {
                        $data['tracking_column'] = 0; 
                    }
                    $this->load->language('sale/order');
                    //code added by gopi to resolve the issue of trackin no. 27 feb--end                         
