...
class ModelCatalogProduct extends Model {
	...
	public function getProducts($data = array(), $exclude_id = null) {
		$sql = "SELECT p.product_id, p.model, p.quantity, (SELECT AVG(rating) AS total FROM " . DB_PREFIX . "review r1 WHERE r1.product_id = p.product_id AND r1.status = '1' GROUP BY r1.product_id) AS rating, (SELECT price FROM " . DB_PREFIX . "product_discount pd2 WHERE pd2.product_id = p.product_id AND pd2.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND pd2.quantity = '1' AND ((pd2.date_start = '0000-00-00' OR pd2.date_start < NOW()) AND (pd2.date_end = '0000-00-00' OR pd2.date_end > NOW())) ORDER BY pd2.priority ASC, pd2.price ASC LIMIT 1) AS discount, (SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special";
		...
		foreach ($query->rows as $result) {
			if ($result['product_id'] != $exclude_id)
    ...
    public function getProductCompare($product_id) {
      $c = false;
	...
	public function getSeller($product_id) {
		$seller = array();
		$seller_id = $this->db->query("SELECT seller_id FROM " . DB_PREFIX . "purpletree_vendor_products WHERE product_id = '" . (int)$product_id . "' LIMIT 1");
		if (isset($seller_id->row['seller_id'])) {
			$seller['id'] = $seller_id->row['seller_id'];
			$seller_address = $this->db->query("SELECT city, address_1 FROM " . DB_PREFIX . "address WHERE customer_id = '" . (int)$seller['id'] . "' LIMIT 1");
			if (isset($seller_address->row['city'])) {
				$seller_city = "г. ".$seller_address->row['city'];
			} else {
				$seller_city = "";
			}
			if (isset($seller_address->row['address_1'])) {
				$seller['address'] = $seller_city.", ".$seller_address->row['address_1'];
			} else {
				$seller['address'] = "";
			}
			$seller_name = $this->db->query("SELECT store_name FROM " . DB_PREFIX . "purpletree_vendor_stores WHERE seller_id = '" . (int)$seller['id'] . "' LIMIT 1");
			if (isset($seller_name->row['store_name'])) {
				$seller['name'] = $seller_name->row['store_name'];
			} else {
				$seller['name'] = "";
			}
			return $seller;
