...
class ModelAccountCustomer extends Model {
	...
	public function getTotalSellersByID($customer_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "kb_mp_seller WHERE customer_id = '" . (int)$customer_id . "'");
		return $query->row['total'];
	}
  
    public function getTotalCustomersByStoretin($storetin){
        $query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "purpletree_vendor_stores WHERE LOWER(store_tin) = '" . $this->db->escape(utf8_strtolower($storetin)) . "'");
