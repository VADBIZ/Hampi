...
class ControllerKbmpMarketplaceOrders extends Controller {
    ...
    public function __construct($params) {
        ...
        $this->model_kbmp_marketplace_kbmp_marketplace->getSellerLoggedStatus($this->url->link('kbmp_marketplace/orders', '', true));
    }
    ...
    protected function getList() {
        ...
        foreach ($results as $result) {
            ...
            $data['orders'][] = array(
                ...
                'invoice' => $this->url->link("kbmp_marketplace/order_details/invoice", "&order_id=" . $result['order_id'], true)
            );
        }
        ...
        if (version_compare(VERSION, '2.2.0.0', '<')) {
            $this->response->setOutput($this->load->view('default/template/kbmp_marketplace/orders.tpl', $data));
        } else {
