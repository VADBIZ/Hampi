...
class ControllerKbmpMarketplaceOrderDetails extends Controller {
    ...
    public function __construct($params) {
        ...
        $this->model_kbmp_marketplace_kbmp_marketplace->getSellerLoggedStatus($this->url->link('kbmp_marketplace/disab', '', true));
    }
    ...
    public function index() {
        ...
        if (!$this->customer->isLogged()) {
            $this->session->data['redirect'] = $this->url->link('kbmp_marketplace/order_details', '&order_id=' . $this->request->get['order_id'], true);
        ...
        //$this->model_kbmp_marketplace_kbmp_marketplace->setSellersEarning(23);
        ...
        if (isset($this->request->get['order_id']) && !empty($this->request->get['order_id'])) {
            ...
            if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
                ...
                if (isset($nonCanceledProducts) && !empty($nonCanceledProducts)) {
                    ...
                    $order_comment = $this->language->get('text_default_comment') . $update_order_status_info['name'] . "\n\n";

                    $products = $this->model_kbmp_marketplace_kbmp_marketplace->getOrderProducts($this->request->get['order_id'], $sellerId['seller_id']);

                    foreach ($products as $product) {
                        //Get Product
                        $product_details = $this->model_kbmp_marketplace_kbmp_marketplace->getProduct($product['product_id']);
                        $order_comment .= $product_details['name'] . "\n";
                    }
                    ...
                    $this->model_kbmp_marketplace_kbmp_marketplace->updateOrderProductStatus($this->request->get['order_id'], $this->request->post['order_status_id'],$sellerId['seller_id']);
                    //End by dharmanshu for myultiple seller order issue 6-02-2021

                    //Add Comment into order history
                    $this->model_kbmp_marketplace_kbmp_marketplace->addOrderHistory($this->request->get['order_id'], $this->request->post['order_status_id'], $sellerId['seller_id'], $order_comment);
            ...
            /* If user tried to access someone else order then redirect to order list page */
            if (empty($order_info)) {
                $this->response->redirect($this->url->link('kbmp_marketplace/orders', '', true));
            }
            ...
            if (isset($order_info) && !empty($order_info)) {
                ...
                //TODO Flag whehter order status change is allowed OR not. As of now, No use of the same but need to add condition like of Order is marked as Completed then don't allow order status change further
                $data['order_change'] = 1;
                ...
                $data['allowed_status'] = array();
                if (!empty($settings['kbmp_marketplace_setting']['allowed_order_status_value'])) {
                    $data['allowed_status'] = $settings['kbmp_marketplace_setting']['allowed_order_status_value'];
                }
                ...
                $histories = $this->model_kbmp_marketplace_kbmp_marketplace->getOrderHistories($this->request->get['order_id'], $sellerId['seller_id']);
    ...
    public function invoice() {
        ...
        $data['kbmp_marketplace_setting'] = $this->model_setting_kbmp_marketplace->getSetting('kbmp_marketplace', $this->config->get('config_store_id'));
        if (isset($this->request->get['order_id']) && !empty($this->request->get['order_id'])) {
            //Get Seller ID
            $sellerId = $this->model_kbmp_marketplace_kbmp_marketplace->getSellerByCustomerId();
            ...
            if ($order_info) {
                ...
                $sellerDetails = $this->model_kbmp_marketplace_kbmp_marketplace->getSeller($sellerId['seller_id']);
                $sellerDetailsLanguage = $this->model_kbmp_marketplace_kbmp_marketplace->getSellerByLang($sellerId['seller_id'], $order_info['language_id']);
                foreach($sellerDetailsLanguage as $key_name => $sellerDetail) {
                    $sellerDetails[$key_name] = $sellerDetail;
                }
                $sellerDetails['country_name'] =  '';
                $sellerDetails['state_name'] =  '';
                if(!empty($sellerDetails['country_id'])) {
                    $country_data = $this->model_localisation_country->getCountry($sellerDetails['country_id']);
                    if(!empty($country_data)) {
                        $sellerDetails['country_name'] =  $country_data['name'];
                     }
                }
                
                if(!empty($sellerDetails['state'])) {
                    $zone_data = $this->model_localisation_zone->getZone($sellerDetails['state']);
                    if(!empty($zone_data)) {
                        $sellerDetails['state_name'] =  $zone_data['name'];
                     }
                }
                $sellerDetails['address'] = nl2br($sellerDetails['address']);
                $data['seller_details'] = $sellerDetails;
    ...
    public function updateTracking() {
        ...
        if ($order_info) {
            if (isset($this->request->get['order_id']) && !empty($this->request->get['order_id'])) {
                if (isset($this->request->get['order_product_id']) && !empty($this->request->get['order_product_id'])) {
                    if (isset($this->request->get['tracking_number']) && !empty($this->request->get['tracking_number'])) {
                        $this->model_kbmp_marketplace_kbmp_marketplace->updateTrackingNumber($this->request->get['order_id'], $this->request->get['order_product_id'], $this->request->get['tracking_number'], $sellerId['seller_id']);
