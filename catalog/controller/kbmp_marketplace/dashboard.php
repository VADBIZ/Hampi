...
class ControllerKbmpMarketplaceDashboard extends Controller {
    ...
    public function __construct($params) {
        ...
        $this->model_kbmp_marketplace_kbmp_marketplace->getSellerLoggedStatus($this->url->link('kbmp_marketplace/dashboard', '', true));
    }
    ...
    public function index() {
        ...
        $data['text_total_eligible_earning'] = $this->language->get('text_total_eligible_earning');
        ...
        //Get Stats - Total Eligible Earning
        $total_eligible_earning_stat = $this->model_kbmp_marketplace_kbmp_marketplace->getTotalSellerEarning('all', 1);
        $data['total_eligible_earning_stat'] = $this->currency->format($total_eligible_earning_stat, $this->session->data['currency']);
        ...
        //Get Stats - Total Payout
        $total_payout_stat = $this->model_kbmp_marketplace_kbmp_marketplace->getTotalSellerPayout($sellerId['seller_id']);
        $data['total_payout_stat'] = $this->currency->format($total_payout_stat, $this->session->data['currency']);

        //Get Stats - Total Credit Amount
        $total_credit_stat = $this->model_kbmp_marketplace_kbmp_marketplace->getTotalCreditAmountPayout($sellerId['seller_id']);
        
        //Total Balance = Earning + Credit - Payment Done
        $total_balance_stat = $total_eligible_earning_stat + $total_credit_stat - $total_payout_stat;
        $data['total_balance_stat'] = $this->currency->format($total_balance_stat, $this->session->data['currency']);
        ...
        if (isset($last_week_earning_graph) && !empty($last_week_earning_graph)) {
            foreach ($last_week_earning_graph as $last_week_earning_graph) {
                $timestamp = strtotime($last_week_earning_graph['DATE(o.date_added)']) * 1000;
                //Last Monday
                if ($timestamp == $lastMonday) {
    ...
    public function request() {
        ...
        if ($sellerIdDetails['approved'] == 2) {
            if ($sellerIdDetails['disapproval_count'] < $sellerIdDetails['approval_request_limit']) {
                $this->model_kbmp_marketplace_kbmp_marketplace->requestApproval($sellerIdDetails['seller_id']);
