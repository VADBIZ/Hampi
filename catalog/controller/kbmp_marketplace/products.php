...
class ControllerKbmpMarketplaceProducts extends Controller {
    ...
    public function __construct($params) {
        ...
        $this->model_kbmp_marketplace_kbmp_marketplace->getSellerLoggedStatus($this->url->link('kbmp_marketplace/products', '', true));
    }
    ...
    public function add() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            if (isset($this->request->get['product_id']) && !empty($this->request->get['product_id'])) {
                ...
                // In case, Seller product approval is required & seller status is not approved then product will be in the disabled status
                ...
                $approval_data = $this->model_kbmp_marketplace_kbmp_marketplace->getProductSeller($this->request->get['product_id']);
                if(!empty($approval_data)){
                    if($approval_data['approved'] == 0 || $approval_data['approved'] == 2){
                        $product_data['status'] = 0;
                    }
                }
                ...
                if (isset($this->request->post['keyword']) && $this->request->post['keyword']) {
                    $this->model_kbmp_marketplace_kbmp_marketplace->edit_seo_keyword($this->request->post['keyword'], $this->request->get['product_id']);
                }
                ...
                if (isset($seller['approved']) && $seller['approved'] != '1') {
                    ...
                    if ($seller_products_total < $seller['product_limit']) {
                        ...
                        // In case, Seller product arrpoval is requried, Add the product in disabled status
                        if (isset($settings['kbmp_marketplace_setting']['kbmp_new_product_approval_required']) && !empty($settings['kbmp_marketplace_setting']['kbmp_new_product_approval_required'])) {
                            $product_data['status'] = 0;
                        }
                        ...
                        // Start BY Dharmanshu for the SEO URL issue 10-07-2020 
                        if (isset($this->request->post['keyword']) && $this->request->post['keyword']) {
                            $this->model_kbmp_marketplace_kbmp_marketplace->insert_seo_keyword($this->request->post['keyword'], $product_id);
                        }
                        // End BY Dharmanshu for the SEO URL issue 10-07-2020
    ...
    protected function getForm() {
        ...
        // If seller account is approved then allow changing the status of the product 
        if (isset($this->request->get['product_id']) && !empty($this->request->get['product_id'])) {
            if ($seller['approved'] == 1) {
                $data['edit'] = true;
            }
        ...
        if (isset($this->request->get['product_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
            ...
            $product_seo = $this->model_catalog_product->getProductSeoUrls($this->request->get['product_id']);
            if (!empty($product_seo)) {
                foreach ($product_seo as $key => $value) {
                    $product_info['keyword'] = array_pop($value);
                    break;
                }
            }
        }
    ...
    public function completeUnassignedCategoryList() {
        ...
        $assignedCategoryList = $this->model_kbmp_marketplace_kbmp_marketplace->getAssignedCategories($this->request->get['seller_id']);
    ...
    protected function validateForm() {
        ...
        if (utf8_strlen($this->request->post['keyword']) > 0) {
            ...
            //condition for loop because the data is saved on the basis of the languages
            if (!empty($url_alias_info)) {
                foreach ($url_alias_info as $k => $v) {
                    if ($url_alias_info && isset($this->request->get['product_id']) && $v['query'] != 'product_id=' . $this->request->get['product_id']) {
