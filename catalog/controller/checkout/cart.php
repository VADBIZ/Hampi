...
//			}
			...
			foreach ($products as $product) {
				...
				if (strlen($product['name']) > 50) {
					$productTitle = utf8_substr(trim(strip_tags(html_entity_decode($product['name'], ENT_QUOTES, 'UTF-8'))), 0, 50) . '...';
					} else {
...
//			}
			...
			$data['total'] = array(
				'title' => $totals[0]['title'],
				'text'  => number_format(intval($totals[0]['value']), 0, '.', ' ') . " ₽"
			);
	...
	public function edit() {
		...
		if (!empty($this->request->post['count'])) {
			foreach ($this->request->post['count'] as $key => $value) {
	...
	public function editTest() {
		...
		$editStatus = $this->cart->update($key, $count);
