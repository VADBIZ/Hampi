...
class ControllerExtensionModulekbmpMarketplace extends Controller {
    ...
    public function install() {
        ...
        $this->model_setting_kbmp_marketplace->assingPageAccess();
    }
    ...
    public function uninstall() {
        ...
        // Module Enable/Disable for Store 0
        $settings = $this->model_setting_kbmp_marketplace->getSetting('kbmp_marketplace', 0);
        $settings['kbmp_marketplace_setting']['kbmp_module_enable'] = 0;
        $this->model_setting_kbmp_marketplace->editSetting('kbmp_marketplace', $settings, 0);
        
        $stores = $this->model_setting_store->getStores();
        if (!empty($stores)) {
            foreach ($stores as $result) {
                $settings = $this->model_setting_kbmp_marketplace->getSetting('kbmp_marketplace', $result['store_id']);
                $settings['kbmp_marketplace_setting']['kbmp_module_enable'] = 0;
                $this->model_setting_kbmp_marketplace->editSetting('kbmp_marketplace', $settings, $result['store_id']);
            }
        }
    }
    ...
    public function index() {
        ...
        $data['heading_subtitle'] = $this->language->get('text_kbmp_general_settings');
        ...
        $data['register_link'] = HTTPS_CATALOG . "index.php?route=account/register&seller_register=1";
        ...
        $data['order_status_value'] = $statusorder;
        
        //Get the module configuration values
        $settings = $this->model_setting_kbmp_marketplace->getSetting('kbmp_marketplace', $store_id);
        if (isset($settings) && !empty($settings)) {
            $data['settings'] = $settings;
			if(!empty($data['settings']['kbmp_marketplace_setting']['cancel_order_status_value'])) {
				$data['order_status_selected'] = $data['settings']['kbmp_marketplace_setting']['cancel_order_status_value'];
			} else {
				$data['order_status_selected'] = array();
			}
            
            if(empty($data['settings']['kbmp_marketplace_setting']['kbmp_default_commission'])) {
				$data['settings']['kbmp_marketplace_setting']['kbmp_default_commission'] = 20;
			}
            
            if(empty($data['settings']['kbmp_marketplace_setting']['kbmp_approval_request_limit'])) {
				$data['settings']['kbmp_marketplace_setting']['kbmp_approval_request_limit'] = 3;
			}
            
            if(empty($data['settings']['kbmp_marketplace_setting']['kbmp_product_limit'])) {
				$data['settings']['kbmp_marketplace_setting']['kbmp_product_limit'] = 10;
			}
        } else {
        ...
        $data['seprate_seller_register'] = $this->language->get('seprate_seller_register');
    ...
    public function common($data = array())
    {
        ...
        $data['text_kbmp_general_settings'] = $this->language->get('text_kbmp_general_settings');
        $data['text_kbmp_order_settings'] = $this->language->get('text_kbmp_order_settings');
        $data['text_sellers_list'] = $this->language->get('text_sellers_list');
        $data['text_kbmp_sellers_list_menu'] = $this->language->get('text_kbmp_sellers_list_menu');
        $data['text_kbmp_sellers_orders_menu'] = $this->language->get('text_kbmp_sellers_orders_menu');
        $data['text_kbmp_sellers_products_products'] = $this->language->get('text_kbmp_sellers_products_products');
        $data['text_kbmp_commission_menu'] = $this->language->get('text_kbmp_commission_menu');
        ...
        $data['text_paypal_payout'] = $this->language->get('text_kbmp_paypal_payout_settings');
        $data['text_email_templates'] = $this->language->get('text_email_templates');
        $data['text_support'] = $this->language->get('text_support');
        $data['text_seller_catgory_commision'] = $this->language->get('text_seller_catgory_commision');

        //links
        $data['general_settings'] = $this->url->link($this->module_path . '/kbmp_marketplace', $this->session_token_key . '=' . $this->session_token, true);
        $data['kbmp_order_settings'] = $this->url->link('kbmp_marketplace/settings/order', $this->session_token_key . '=' . $this->session_token, true);
        $data['sellers_account_approval_list'] = $this->url->link('kbmp_marketplace/sellers_account_approval_list', $this->session_token_key . '=' . $this->session_token, true);
        $data['products_approval_list'] = $this->url->link('kbmp_marketplace/products_approval_list', $this->session_token_key . '=' . $this->session_token, true);
        $data['products_list'] = $this->url->link('kbmp_marketplace/products_list', $this->session_token_key . '=' . $this->session_token, true);
        $data['sellers_list'] = $this->url->link('kbmp_marketplace/sellers_list', $this->session_token_key . '=' . $this->session_token, true);
        $data['sellers_order'] = $this->url->link('kbmp_marketplace/sellers_order', $this->session_token_key . '=' . $this->session_token, true);
        $data['admin_order'] = $this->url->link('kbmp_marketplace/admin_order', $this->session_token_key . '=' . $this->session_token, true);
        $data['products_review'] = $this->url->link('kbmp_marketplace/products_review', $this->session_token_key . '=' . $this->session_token, true);
        $data['sellers_review_approval_list'] = $this->url->link('kbmp_marketplace/sellers_review_approval_list', $this->session_token_key . '=' . $this->session_token, true);
        $data['sellers_review'] = $this->url->link('kbmp_marketplace/sellers_review', $this->session_token_key . '=' . $this->session_token, true);
        $data['sellers_category_request'] = $this->url->link('kbmp_marketplace/sellers_category_request', $this->session_token_key . '=' . $this->session_token, true);
        $data['sellers_shipping'] = $this->url->link('kbmp_marketplace/sellers_shipping', $this->session_token_key . '=' . $this->session_token, true);
        $data['seller_category'] = $this->url->link('kbmp_marketplace/seller_category', $this->session_token_key . '=' . $this->session_token, true);
        $data['admin_commission'] = $this->url->link('kbmp_marketplace/admin_commission', $this->session_token_key . '=' . $this->session_token, true);
        $data['sellers_transaction'] = $this->url->link('kbmp_marketplace/sellers_transaction', $this->session_token_key . '=' . $this->session_token, true);
        $data['sellers_balance'] = $this->url->link('kbmp_marketplace/sellers_balance', $this->session_token_key . '=' . $this->session_token, true);
...
//        $data['support'] = $this->url->link('kbmp_marketplace/support', $this->session_token_key . '=' . $this->session_token, true);
        ...
        if (VERSION >= '2.2.0.0') {
            return $this->load->view('kbmp_marketplace/menu', $data);
        } else {
            return $this->load->view('kbmp_marketplace/menu.tpl', $data);
        }
    }
}
