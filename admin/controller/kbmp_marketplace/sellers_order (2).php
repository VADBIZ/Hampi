...
class ControllerKbmpMarketplaceSellersOrder extends Controller {
    ...
    protected function getList() {
        ...
        if (isset($this->request->get['filter_delievery_status'])) {
            $filter_delievery_status = $this->request->get['filter_delievery_status'];
        } else {
            $filter_delievery_status = 3;
        }
        ...
        if (isset($this->request->get['filter_delievery_status'])) {
            $url .= '&filter_delievery_status=' . urlencode(html_entity_decode($this->request->get['filter_delievery_status'], ENT_QUOTES, 'UTF-8'));
        }
        ...
        $filter_data = array(
            ...
            'filter_delievery_status'    => trim($filter_delievery_status),
...
$data['text_support'] = $this->language->get('text_support');
        ...
        /*
            Start BY Dharmanshu 28-08-2020 for the custom change
        */
        $data['delievery_yes'] = $this->language->get('delievery_yes');
        $data['delievery_select_yes'] = $this->language->get('delievery_select_yes');
        $data['delievery_no'] = $this->language->get('delievery_no');
        $data['delievery_select_no'] = $this->language->get('delievery_select_no');
        $data['delievery_status_change'] = $this->language->get('delievery_status_change');
        /*
            End BY Dharmanshu 28-08-2020 for the custom change
        */
        $url = '';
        ...
        $data['sort_delievery_status'] = $this->url->link('kbmp_marketplace/sellers_order', $this->session_token_key.'=' . $this->session_token . '&sort=ksod.is_delivered' . $url, true);
       ...
       /*
            Start BY Dharmanshu 28-08-2020 for the custom change
        */
        $data['delievery_status_link'] =  $this->url->link('kbmp_marketplace/sellers_order/change_delievery_status', $this->session_token_key.'='. $this->session_token, true);
        /*
            End BY Dharmanshu 28-08-2020 for the custom change
        */
        $url = '';
    ...
    /*
            Start BY Dharmanshu 28-08-2020 for the custom change for chnaging status of the delievery
        */
    public function change_delievery_status(){
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
            $order_id = $this->request->post['order_id'];
            $this->load->model('kbmp_marketplace/kbmp_marketplace');
            $this->model_kbmp_marketplace_kbmp_marketplace->change_delievery_status($order_id);
            echo 1;
        }
        /* echo '<pre>';
        print_r($_POST);
    ...
    /*
            ENd BY Dharmanshu 28-08-2020 for the custom change for chnaging status of the delievery
        */
}
