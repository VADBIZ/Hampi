...
class ControllerKbmpMarketplaceSellerPayoutRequest extends Controller {
    ...
    protected function getList() {
        ...
        if (isset($settings['kbmp_marketplace_paypal_settings']['enable']) && $settings['kbmp_marketplace_paypal_settings']['enable']) {
            $data['paypal_payout_enabled'] = 1;
        } else {
            $data['paypal_payout_enabled'] = 0;
        }
        ...
        $active_tab['active_tab'] = 44;
        $data['tab_common'] = $this->load->controller($this->module_path . '/kbmp_marketplace/common', $active_tab);

        if (VERSION < 2.2) {
            $this->response->setOutput($this->load->view('kbmp_marketplace/seller_payout_request.tpl', $data));
        } else {
    ...
    public function approve() {
        ...
        $settings = $this->model_setting_kbmp_marketplace->getSetting('kbmp_marketplace', $this->config->get('config_store_id'));
        if (isset($settings['kbmp_marketplace_paypal_settings']['enable']) && $settings['kbmp_marketplace_paypal_settings']['enable']) {
            $paypal_payout_enabled = 1;
        } else {
            $paypal_payout_enabled = 0;
        }
        
        if (isset($this->request->post['id']) && $this->request->post['id'] != '') {
            if ($this->request->post['transaction_type'] == 'paypal' && $paypal_payout_enabled == 1) {
