...
class ControllerKbmpMarketplaceAdminOrder extends Controller {
    ...
    public function index() {
        ...
        $this->load->language('kbmp_marketplace/admin_order');
    ...
    protected function getList() {
        ...
        $admin_order_total = $this->model_kbmp_marketplace_kbmp_marketplace->getTotalAdminOrders($filter_data);

        $data['admin_order_total'] = $admin_order_total;
        
        $results = $this->model_kbmp_marketplace_kbmp_marketplace->getAdminOrders($filter_data);

        foreach ($results as $result) {
            
            $data['admin_order'][] = array(
        ...
        $pagination->total = $admin_order_total;
        $pagination->page = $page;
        $pagination->limit = $this->config->get('config_limit_admin');
        $pagination->url = $this->url->link('kbmp_marketplace/admin_order', $this->session_token_key.'=' . $this->session_token . $url . '&page={page}', true);

        $data['pagination'] = $pagination->render();

        $data['results'] = sprintf($this->language->get('text_pagination'), ($admin_order_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($admin_order_total - $this->config->get('config_limit_admin'))) ? $admin_order_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $admin_order_total, ceil($admin_order_total / $this->config->get('config_limit_admin')));
        ...
        $active_tab['active_tab'] = 32;
        $data['tab_common'] = $this->load->controller($this->module_path . '/kbmp_marketplace/common', $active_tab);

        if (VERSION < 2.2) {
            $this->response->setOutput($this->load->view('kbmp_marketplace/admin_order.tpl', $data));
        } else {
            $this->response->setOutput($this->load->view('kbmp_marketplace/admin_order', $data));
        }
    }
}
