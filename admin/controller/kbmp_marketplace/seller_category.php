...
class ControllerKbmpMarketplacesellerCategory extends Controller {
    ...
    protected function getList() {
        ...
        $active_tab['active_tab'] = 17;
        $data['tab_common'] = $this->load->controller($this->module_path . '/kbmp_marketplace/common', $active_tab);

        if (VERSION < 2.2) {
            $this->response->setOutput($this->load->view('kbmp_marketplace/seller_category.tpl', $data));
        } else {
    ...
    public function addCategoryCommission() {
        ...
        if (isset($this->request->get['commission'])) {
            ...
            $this->document->setTitle($this->language->get('edit_rule'));
        } else {
